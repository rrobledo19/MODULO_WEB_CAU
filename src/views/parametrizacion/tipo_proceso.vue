            ...
            <template class="row">
              <div class="row">
                ...
                >
                  <b-form-input
                    id="P_DSCRPCION"
                    v-model="datos_procesos.P_DSCRPCION"
                    placeholder="Descripcion Proceso"
               ...
               <!-- Tipo de plantilla  -->
                ...
                >
                  <b-form-select
                    id="P_ID_IMPSTO"
                    v-model="datos_procesos.P_ID_IMPSTO"
                ...
                <b-form-group
                  ...
                  label-for="Tipo Plantilla"
                >
                  <b-form-select
                    id="P_TPO_PLNTLLA"
                    v-model="datos_procesos.P_TPO_PLNTLLA"
...
export default {
  ...
  data() {
    return {
      fields: [
      /*  { key: "id_tpo_prcso", label: "#", sortable: true },*/
        { key: "dscrpcion", label: "Proceso", sortable: true },
        { key: "tpo_plntlla", label: "Pantilla", sortable: true },
        { key: "id_impsto", label: "Impuesto", sortable: true },
      ...
      mensage_1 :"",
      titulo_general:"Registro Información Tipo Proceso",
      manejador:"", 
  ...
  methods: {
    ...
    get_proceso() {
      let url = "http://localhost:3000/tipo_proceso/lista_tipo_procesos";
      const proceso = async () => {
        try {
        ...
        console.log("Respuestas del get", proceso);
      };
...
get_impuesto() {
      ...
      const v_impuestos = async () => {
        try {
          const resp = await axios.get(url);

          resp.data.map((imp) => {
             console.log('datos proceso',imp);

          let imps = {
              id_impuestos:      imp.id,
              nombre_impuestos:  imp.nmbre_impsto,   
            };

            this.impuestos.push({
              value: imps.id_impuestos,
              text:  imps.nombre_impuestos,
            });
          });
        } catch (error) {
   ...
   addproceso(){
    let url = "http://localhost:3000/tipo_proceso/registro_tipo_procesos";
    this.datos_procesos.P_ACCION = "R";
    this.titulo = "Registro Información Tipo de Proceso";
    this.tipo_accion = true;

    const v_proceso = async()=> {
       try {
         console.log('entrando en datos procesos',this.datos_procesos )
         resp = await axios.post(url,this.datos_procesos);
         console.log('Repuesta del servidor de procso',resp) ;
         ...
         console.log('error al registrar procesos',error); 
       }
    ...
    Registro_informacion() {
     ...
     adicionar_datos(item) {
      ...
      this.datos_procesos.P_DSCRPCION = item.dscrpcion; 
      this.datos_procesos.P_ID_TPO_PRCSO = item.id_tpo_prcso
      this.datos_procesos.P_TPO_PLNTLLA =item.tpo_plntlla; 
      this.datos_procesos.P_ID_IMPSTO = item.id_impsto; 
      this.tipo_accion = false;
      this.titulo = "Actualizar Información Tipo Procesos";
    ...
    updateproceso(){
      this.datos_procesos.P_ACCION = "A";
      this.titulo = "Actualizar Información de Impuestos";
      console.log("entrando al update informacion", this.datos_procesos);
    

      let url = "http://localhost:3000/tipo_proceso/actualizar_tipo_proceso"; 
      const v_proceso = async()=>{
        try {
         const resp = axios.post(url,this.datos_procesos);
        console.log("entrando al update informacion",resp);
         if (!resp.data.outBinds.O_VLDA) {
            this.msg_title   = "Notificación"
          ...
          console.log('Error al actualizar tipo proceso',error); 
        }
      }
    ...
    eliminarproceso(r_item) {
      let url = "http://localhost:3000/tipo_proceso/eliminar_tipo_proceso";
      this.datos_procesos.P_ACCION = "";
      this.datos_procesos.P_ID_TPO_PRCSO = r_item.id_tpo_prcso; 
 
      const v_proceso = async () => {
        try {
        const resp = await axios.post(url, this.datos_procesos);
 ...
 cancelar_informacion(){
      this.datos_procesos.P_DSCRPCION = ""; 
        this.datos_procesos.P_TPO_PLNTLLA =""; 
        this.datos_procesos.P_ID_IMPSTO = "";   
      this.$refs.modal.hide("moda_Registro");
    }, 
